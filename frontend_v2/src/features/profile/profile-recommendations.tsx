    ...
    <section className="profile-recommendation" aria-labelledby="profile-recommendation-title">
      <div className="profile-recommendation-heading">
        <div>
          ...
          <span>This score reflects overlap between saved keywords and demo research topics. It is not an admission probability.</span>
        </div>
      ...
      {keywords.length === 0 ? (
        ...
        <div className="profile-recommendation-empty"><strong>No demo professors have an exact keyword match</strong><p>Try a research area filter or different keyword wording in Professor search.</p><Link href="/professors">Search with filters</Link></div>
      ) : (
        <ol className="profile-recommendation-list">
          {recommendations.map(({ lab, matchingTopics }) => (
            <li key={lab.id}>
              <div className="profile-recommendation-card-topline"><span>Demo data</span><small>{lab.institution}</small></div>
