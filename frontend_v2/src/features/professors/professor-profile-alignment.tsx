...
import { getProfile, type UserProfile, WorkspaceApiError } from "../workspace/api";
...
export function ProfessorProfileAlignment({ topics, variant }: ProfessorProfileAlignmentProperties) {
  ...
  useEffect(() => {
    ...
    void getProfile().then((value) => {
      ...
      if (!active) return;
      if (error instanceof WorkspaceApiError && error.status === 404) setProfile(null);
      else setFailed(true);
    }).finally(() => {
  ...
  if (variant === "summary") {
    ...
    if (profile === null) return <><strong>Create a profile to compare</strong><p>Add research keywords and a CV for evidence-based matching.</p></>;
