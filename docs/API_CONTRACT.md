...
### Implemented lab search query details

`GET /labs` accepts optional `university`, `department`, repeated `field`,
`q`, `professor_name`, and `lab_name` filters. Filters are combined with AND;
repeated `field` values are combined with OR. `q` searches Korean and English
keyword terms along with lab, professor, field, and summary text. Omitting all
filters returns every lab.

`sort` is `score` (descending persisted `Recommendation.total_score`, then
latest update) or `recent` (latest update first). It defaults to `recent`.
`page` is one-based and `page_size` defaults to 20. Invalid values return the
standard 422 validation response. List items include nullable
`recommendationScore` and `isFavorite`, evaluated for the MVP `demo-user`
context. `GET /labs/{lab_id}` includes `isFavorite` as well.
